...
public sealed class CapacityStrategy : ICapacityStrategy
{
    ...
    /// <exception cref="InvOpException">Thrown if the required size exceeds the maximum value in the capacity array.</exception>
    public int CalculateCapacity(int currentCapacity, int requiredSize)
    {
        ...
        throw new InvOpException("Required capacity is too large.",
            errorData: new { CurrentCapacity = currentCapacity, RequiredSize = requiredSize });
    }
}
